...
export default class RenderFilm extends Component {
    ...
    render() {
        return (//Sử dụng css rồi gán lại
            <div className='background-film'>
                <div style={{ width: '100%', minHeight: '100vh', top: 0, left: 0, background: 'rbga(0,0,0,.5)' }}>
                    ...
                    <div className="list-film mt-2 row">
                        {this.renderFilm()}
                    </div>
                </div>
            </div>
    ...
    renderFilm = () => {
        return arrFilm.map((film, index) => {
            return <div className="col-3 mt-2" key={index}>
                <div className="card">
                    <img src={film.hinhAnh} className="card-img-top" alt="..." height={350} style={{ objectFit: 'cover' }} />
                    {/* Sử dụng toán tử 3 ngôi ...?...: để setup ... khi mô tả dài */}
                    <div className="card-body">
                        <h4 className="card-title" style={{ height: '50px' }}>{film.tenPhim}</h4>
                        <p className="card-text" style={{ height: '100px', fontSize: '13px' }}>{film.moTa.length > 200 ? film.moTa.substring(0, 200) + '...' : film.moTa}</p>
                        <p className="card-text text-danger">{film.ngayKhoiChieu}</p>
                        <a href={film.trailer} className="btn btn-primary">Trailer</a>
                    </div>
                </div>
            </div>
        })
